...
def main() -> int:
    ...
    assert ".class-diagram" in styles and ".class-package" in styles, "Class package diagram styles are missing"
    assert "function buildClassPackageLayout" in app and "function buildClassPackages" in app, "UML package-style class-map layout is missing"
    assert "function aggregatePackageRelations" in app and "sourcePackageId === targetPackageId" in app, "Class-map relations must aggregate by package and keep internal relations inside packages"
    assert ".class-package-edge path" in styles and "stroke-dasharray" in styles, "Class-map package dependencies must use dashed UML-style connectors"
